"""
Storing all the information about the current state of ChessPuzzleGenerator game.
...
class GameState:
    ...
    def getBlackKingLocation(self, board):
        for i in range (8):
            for j in range(8):
                if board[i][j] == 'bK':
                    return (i,j)
...
class Move:
    # in ChessPuzzleGenerator, fields on the board are described by two symbols, one of them being number between 1-8 (which is corresponding to rows)
    ...
    # to match the ones used in the original ChessPuzzleGenerator game
